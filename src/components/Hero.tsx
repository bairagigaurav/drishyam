...
export default function Hero() {
  ...
  const slides = siteContent.hero.slides ?? DEFAULT_SITE_CONTENT.hero.slides;
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              ...
              fill
              priority
              unoptimized={typeof activeSlide.image === "string" && activeSlide.image.startsWith("data:")}
              className="object-cover opacity-75 group-hover:scale-105 transition-transform duration-1000 ease-out"
              sizes="100vw"
            />
          </motion.div>
        </AnimatePresence>
              ...
              <div className="relative w-full max-w-[420px] rounded-[28px] border border-white/10 bg-white/8 p-4 shadow-[0_30px_80px_rgba(15,23,42,0.45)] backdrop-blur-md">
                <div className="overflow-hidden rounded-[22px] bg-[#f8fafc]">
                  <div className="relative aspect-[4/5]">
                    <Image
                      src={slides[(activeIndex + 1) % slides.length]?.image ?? activeSlide.image}
                      alt={slides[(activeIndex + 1) % slides.length]?.alt ?? activeSlide.alt}
                      fill
                      unoptimized={true}
                ...
                <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-[#111827] px-4 py-3 text-white">
                  <div>
                    <p className="text-[10px] uppercase -[0.24em] text-slate-300">{activeSlide.badge || "Best Seller"}</p>
                    <p className="mt-1 text-lg font-semibold">{activeSlide.title}</p>
                  </div>
                  <span className="rounded-full bg-[#f59e0b] px-3 py-1 text-xs font-bold text-[#111827]">Special Offer</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {slides.map((slide, index) => (
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full transition-all ${
                index === activeIndex ? "w-9 bg-[#f59e0b]" : "bg-white/60 hover:bg-white"
              }`}
            />
          ))}
        </div>
